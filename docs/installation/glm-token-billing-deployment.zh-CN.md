...
`data/token_billing_tiers.json` 支持热加载开关：
...
- `enabled` 默认为 `true`。设置为 `false` 后，GLM 梯度计费完全关闭。
- `reload_interval_seconds` 默认为 `15` 秒。程序不会每次请求都读取文件，只会最多每 15 秒检查一次配置文件修改时间。
- 配置解析失败时会保留上一份有效配置，避免生产计费规则被错误 JSON 清空。
